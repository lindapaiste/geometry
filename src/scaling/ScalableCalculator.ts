...
const x: Limits<'height' | 'width', I_Sized> = {
    //width: 200,
    height: undefined,
};

//<OT extends Record<string, any>, Scalable extends keyof OT>
...
export class ScaleCalculator<Scalable extends string, OT extends Record<Scalable, number>> {
    ...
    /**
     * @param {object} propertyMaximums is an object where the keys are the property names and the values are the maximum value for that property
     * @return {number} the largest scale for which all properties are less than the provided maximum
     */
    calcScaleToFit(propertyMaximums: Limits<Scalable, OT> ): number {
        //in order to meet all conditions, we want the MINIMUM
        return Math.min( ...this._getScalesFromLimits( propertyMaximums ) );
    }

    /**
     * @param {object} propertyMinimums is an object where the keys are the property names and the values are the minimum value for that property
     * @return {number} the smallest scale for which all properties are greater than the provided minimum
     */
    calcScaleToCover(propertyMinimums: Limits<Scalable, OT> ): number {
    ...
    private _getScalesFromLimits( limits: Limits<Scalable, OT> ): number[] {
        ...
        this.scalableProperties.forEach( propertyName => {
           const limitVal = limits[propertyName] || undefined; //can do this whether or not the property exists because I defined Limits to be string-indexed
