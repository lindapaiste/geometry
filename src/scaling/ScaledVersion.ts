 ...
 */
export const defaultApplyScaledValues = <
 ...
 */
export type ApplyScaledValues<
  Scalable extends string,
  OT extends Record<Scalable, number>
> = (values: Required<Pick<OT, Scalable>>, original: OT) => OT;

export default class ScaledVersionCreator<
  Scalable extends string,
  OT extends Record<Scalable, number>
> implements IScalableObject<Scalable, OT> {
  ...
  constructor(
    ...
    object: OT,
    applyScaledValues: ApplyScaledValues<
      Scalable,
      OT
    > = defaultApplyScaledValues
  ) {
